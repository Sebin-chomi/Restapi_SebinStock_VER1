# config.py
"""
⚙️ 전역 설정 파일
- 실전/모의 전환
- API 키 / 서버
- 매수 시간 필터
- 손절 / 트레일링 파라미터
- 리스크 관리 (MDD 등)
"""

# =====================================================
# 🔁 실전 / 모의투자 전환
# =====================================================
# ⚠️ 실전으로 바꿀 때만 False
is_paper_trading = True

# =====================================================
# 🌱 환경 식별 (로그/표시용)
# =====================================================
ENV_NAME = "TEST"


# =====================================================
# 🔐 API KEY (따옴표 필수)
# =====================================================
# 실전
real_app_key = "(실전 APP KEY)"
real_app_secret = "(실전 APP SECRET)"

# 모의
paper_app_key = "-ty84nqoUT_lqsCDBdzWDwlXhY2GD3gSVTISF634OG0"
paper_app_secret = "AQcFnDAqDisMozC8zjilrBnJBFpnRRgljTxgxmAFucM"


# =====================================================
# 🌐 API / SOCKET URL
# =====================================================
real_host_url = "https://api.kiwoom.com"
paper_host_url = "https://mockapi.kiwoom.com"

real_socket_url = "wss://api.kiwoom.com:10000"
paper_socket_url = "wss://mockapi.kiwoom.com:10000"

# 선택된 환경
app_key = paper_app_key if is_paper_trading else real_app_key
app_secret = paper_app_secret if is_paper_trading else real_app_secret
host_url = paper_host_url if is_paper_trading else real_host_url
socket_url = paper_socket_url if is_paper_trading else real_socket_url


# =====================================================
# 💰 예수금 설정 (실전 / 모의 공통 제어)
# =====================================================

# 모의투자에서 사용할 가짜 예수금
PAPER_FAKE_CASH = 3_000_000  # 300만원



# =====================================================
# 🧪 기존 코드 호환용 alias
# (❗ 판단 기준은 is_paper_trading 하나만)
# =====================================================
FAKE_MODE   = is_paper_trading
TEST_MODE   = is_paper_trading
PAPER_CASH  = PAPER_FAKE_CASH
FAKE_CASH   = PAPER_FAKE_CASH

# =====================================================
# 💰 전략 자금 제한
# =====================================================
STRATEGY_MAX_CASH = PAPER_FAKE_CASH

# =====================================================
# 🔧 기타
# =====================================================
DEBUG_MODE = True


# =====================================================
# 🕒 매수 시간 필터 (전략 환경 조건)
# =====================================================
# 장 초반 / 마감 직전 제외
BUY_START_TIME = "0930"
BUY_END_TIME = "1430"


# =====================================================
# 📈 매수 전략 관련 파라미터
# =====================================================
# 거래량 기준봉 배수 (평균 거래량 대비)
ANCHOR_VOLUME_MULT = 5


# =====================================================
# 🛑 손절 전략 (Fail Fast)
# =====================================================
# 박스 높이 대비 손절 buffer 비율
# 손절가 = box_low - (box_height * ratio)
STOPLOSS_BOX_BUFFER_RATIO = 0.20


# =====================================================
# 🟦 트레일링 스탑 전략
# =====================================================
# 트레일링 활성화 조건 A: 수익률 기준
TRAIL_ACTIVATE_PROFIT_PCT = 0.015  # +1.5%

# 트레일링 활성화 조건 B: 박스 상단 위 유지 봉 수
TRAIL_ACTIVATE_HOLD_BARS = 3

# 트레일링 가격 계산 기준 봉 수
TRAIL_N_BARS = 3


# =====================================================
# 📉 리스크 관리 (계좌 단위)
# =====================================================
# 일중 최대 손실률 (MDD)
# 예: -10% 도달 시 자동 중단
MAX_DAILY_MDD_PCT = -0.10


# =====================================================
# ⏱ 재진입 제어
# =====================================================
# 손절 후 재진입 쿨타임 (분)
STOPLOSS_COOLDOWN_MIN = 30

# =====================================================
# 🔎 조건검색 관리
# =====================================================
# 조건검색으로 동시에 추적할 최대 종목 수
MAX_CONDITION_STOCKS = 20  # 10 ~20 : 안정적, 30 이상 : 관리부담증가

# =====================================================
# ⏳ 조건검색 → 매수 대기 제한
# =====================================================
# 조건검색 포착 후 최대 대기 시간 (초)
# 이 시간 초과 시 매수 시도 없이 후보에서 제거
MAX_WAIT_BEFORE_BUY_SEC = 300  # 300 (5분), 180(3분):아주타이트, 600(10분):느슨


# =====================================================
# 📊 리포트 / 로그
# =====================================================
# 일일 손익 로그 파일
DAILY_PNL_LOG_PATH = "./logs/daily_pnl.csv"

# 월간/주간 요약 저장 경로
SUMMARY_REPORT_PATH = "./reports/"


# =====================================================
# 📩 텔레그램
# =====================================================
telegram_chat_id = "8254571263"
telegram_token = "7995086247:AAGIueTwRd_aHFZiodWoEqK2luGq4OcA034"

# ======================================================
# 💳 계좌 정보
# ======================================================
# 모의투자 / 실전 공통 (형식: "12345678-01")
account_no = "00000000-01"  # ← 지금은 더미값 (모의투자용)
account_product = "01"  # 계좌 상품 코드 (보통 "01")

# ======================================================
# 🧾 TR ID (주문용)
# ======================================================
# 모의투자 / 실전 주문 TR ID
BUY_TR_ID = "VTTC0802U" if is_paper_trading else "TTTC0802U"
SELL_TR_ID = "VTTC0801U" if is_paper_trading else "TTTC0801U"

# risk_manager 호환용 별칭
MAX_MDD_PCT = MAX_DAILY_MDD_PCT

# =====================================================
# 🛑 자동매매 중단(HALT) 플래그 파일
# =====================================================
HALT_FLAG_FILE = "./halt.flag"

# =====================================================
# 🛑 TEST_MODE : 장시간 무시, 매수 조건 완화옵션
# =====================================================

TEST_MODE = True  # False면 실전

BUY_START_TIME = "0930"
BUY_END_TIME = "1430"

MIN_SIGNAL_COUNT_TEST = 1
MIN_SIGNAL_COUNT_REAL = 2


# =====================================================
# 📈 매수 가격 전략 (앞에서부터 순서대로 시도)
# 기본값은 기존 동작과 동일
# =====================================================

BUY_PRICE_STRATEGY = [
    "MARKET",  # 시장가
    "ASK+1",  # 매도호가 1호가
    # "ASK+2",  # ← 월요일 이후에 켜면 됨
]

# =====================================================
# 📉 매도 가격 전략 (앞에서부터 순서대로 시도)
# 기본값은 기존 동작과 동일
# =====================================================

SELL_PRICE_STRATEGY = [
    "MARKET",  # 시장가
    "BID-1",  # 매수호가 1호가
    # "BID-2",  # ← 월요일 이후에 켜면 됨
]

# =====================================================
# 🔁 주문 실패 재시도 설정
# =====================================================

ORDER_RETRY_ENABLED = False  # false는 비활성, 실전시 true로 변경
ORDER_RETRY_COUNT = 2  # 최대 재시도 횟수
ORDER_RETRY_DELAY_SEC = 1  # 재시도 간 대기 (초)


# =====================================================
# 💰 전략용 자금 제한 (모의/실전 공통)
# =====================================================
STRATEGY_MAX_CASH = 3_000_000  # 실전 기준 300만원


# =====================================================
# 💓 텔레그램 하트비트 설정
# =====================================================
HEARTBEAT_INTERVAL_MIN = 30  # 몇 분마다 생존 알림 보낼지 (30분)


# ======================================================
# 💳 계좌 정보 (공통 표준 이름)
# ======================================================
ACCOUNT_NO = account_no.replace("-", "")




def get_available_cash(real_cash: int | None = None) -> int:
    """
    실전/모의에 따라 사용할 예수금 반환
    - 실전: balance API 결과 사용
    - 모의: 고정 가짜 예수금 사용
    """
    if is_paper_trading:
        return PAPER_FAKE_CASH

    if real_cash is None:
        raise ValueError("실전 거래인데 real_cash가 None입니다")

    return real_cash

# TEST/config_test.py

ENV_NAME = "TEST"   # 단순히 가상환경이름 지정해둔거


# ==================================================
# 🧪 TEST 전용 옵션
# ==================================================
FORCE_FAKE_SELL = True
