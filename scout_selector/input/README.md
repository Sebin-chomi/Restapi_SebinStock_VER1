# Scout Selector Input Format

외부 신호(조건식·뉴스)를 selector가 사용하는 표준 입력 포맷
...
**selector는 입력 원천을 모른다**  
**selector는 오직 theme_score_map만 받는다**  
...
input_dir = Path("scout_selector/input/conditions")
...
input_dir = Path("scout_selector/input/news")
input_dir.mkdir(parents=True, exist_ok=True)
...
과거 신호 파일은 자동으로 `scout_selector/history/input/` 디렉토리로 이동됩니다:
